...
SitemapController.prototype = {
    ...
    _init : function () {
        this.loadTemplates(function () {
            ...
            this.control({
                ...
                "#edit-selector button[action=auto-selector]": {
		...
		for (var selector in controls) {
			for (var event in controls[selector]) {
				$(document).on(event, selector, (function (selector, event) {
					return function () {
						var continueBubbling = controls[selector][event].call(_this, this);
	...
	initMultipleStartUrlHelper: function () {
		$("#startUrl")
			.popover({
				title: '多个入口 URL',
				html: true,
				content: "你可以创建一个正则范围的入口 URL，例如:<br />http://example.com/[1-100].html",
				placement: 'bottom'
			})
			.blur(function () {
				$(this).popover('hide');
			...
			fields: {
				...
				'_id': {
		          validators: {
		            ...
		            regexp: {
		              regexp: /^[a-z][a-z0-9_$()+\-/]+$/,
		              message: '网站 ID 只支持字母 (a-z), 数字 (0-9), 或者 _, $, (, ), +, -, /。 并且必须以字母开头。'
		            },
		            callback: {
		              message: '网站 ID 已经存在',
	...
	showCreateSitemap: function () {
		...
		var _domain = this.domainName(this.currentUrl);

        $("#viewport form input[name=_name]").val(_domain);
        $("#viewport form input[name=_id]").val('md5_' + md5(_domain));
        $("#viewport form input[name=startUrl\\[\\]]").val(this.currentUrl);
	...
	createSitemap: function (form) {
        ...
        this.console.log("crontroller receive sitemapExists");
	...
	saveSitemapExport: function (button) {
        this.console.log("createSitemapUpload start...");
	...
	_editSelector: function (selector) {
        ...
        var _domainName = this.domainName(this.currentUrl);
        var _taskName = 't_' + _domainName + '_' + (Math.round(Math.random() * 900) + 100);
        $("#selectorId").val(_taskName);
        $("#selectorName").val(_taskName);
	...
	selectSelector: function (button) {
		...
		var _sitemap = this.getCurrentlyEditedSelectorSitemap();
		var selector = this.getCurrentlyEditedSelector();
		var currentStateParentSelectorIds = this.getCurrentStateParentSelectorIds();
		var parentCSSSelector = _sitemap.selectors.getParentCSSSelectorWithinOnePage(currentStateParentSelectorIds);
    ...
    autoSelectSelector: function (button) {
        ...
        var deferredSelector = this.contentScript.autoSelector({
            parentCSSSelector: parentCSSSelector,
            allowedElements: selector.getItemCSSSelector()
        });
    },
    ...
    autoTableDataRowSelector: function(button) {
        ...
        var selector = this.getCurrentlyEditedSelector();
        var currentStateParentSelectorIds = this.getCurrentStateParentSelectorIds();
        var parentCSSSelector = _sitemap.selectors.getCSSSelectorWithinOnePage(selector.id, currentStateParentSelectorIds);
